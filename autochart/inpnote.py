...
ALL_JUMPS = [(Panel.L, Panel.R), (Panel.L, Panel.U), (Panel.L, Panel.D), (Panel.D, Panel.U), (Panel.D, Panel.R), 
    (Panel.U, Panel.D), (Panel.U, Panel.R)]

ALL_CORNERS = [(Panel.L, Panel.U), (Panel.L, Panel.D), (Panel.D, Panel.R), 
    (Panel.U, Panel.R)]

player = Player()

def unpackPhrases(phrases): return [measure for phrase in phrases
                                    for measure in phrase]

def isBeatCode(char): return char.isalpha() or char == '.'
...
def flippedPhrase(phrase, direction):
    ...
    for measure in phrase: measures.append([flip_function(beat) for beat in measure])
    ...
    while ii < len(line):
        ...
        match char:
            ...
            case "j":
                ...
                skip_beat = True
            case "l" | "o":
                note_kind = matchVal(char, ['d', NoteKind.Hold], ['o', NoteKind.Roll])
...
def nextRegNote(options, length = -1, note_kind = NoteKind.Tap):
    ...
    if options.horizontal: banned_panels |= {Panel.U, Panel.D}

    if options.vertical: banned_panels |= {Panel.L, Panel.R}

    if options.crossover: 
        banned_panels |= set(allPanelsBut([crossoverPanel(step_foot)]))
        crossDirection(step_foot)

    elif not options.side_switch: banned_panels.add(crossoverPanel(step_foot))

    if options.weak_uncross: banned_panels |= set(allPanelsBut([weakUncrossPanel(step_foot)]))

    if options.strong_uncross: banned_panels |= set(allPanelsBut([strongUncrossPanel(step_foot)]))

    if options.weak_uncross or options.strong_uncross: player.direction = Facing.U

    if options.move_off: banned_panels |= set(beforeLastFootBeat().getPanels())

    if options.repeat: banned_panels |= set(allPanelsBut(beforeLastFootBeat().getPanels()))

    if options.switch: banned_panels |= {Panel.L, Panel.R}
    ...
    if options.bracket: choices = chooseBracket(valid_choices)
    else: choices = [weightedChoice(valid_choices, [step_foot_panel, 0.5])]
...
def nextJump(length, note_kind, corner = False, opposite = False):
    ...
    if opposite: 
        while jump[0] in occupied_panels or jump[1] in occupied_panels: jump = randomJump(corner) 
    else:
        while jump[0] in occupied_panels and jump[1] in occupied_panels: jump = randomJump(corner) 
...
def chooseBracket(panels):
    ...
    return (random.choice(vert_panels), random.choice(hori_panels))
...
def padMeasureWithMines(measure, mines_and_indices):
    ...
    unpacked_measure = [beat for beats in new_measure
                                for beat in beats]

    for mine_beat, ii in mines_and_indices: unpacked_measure[ii * 2 - 1] = mine_beat
...
def outOfWayMine():
    ...
    chosen_panel = random.choice(valid_choices)

    mine_beat = Beat([Note(chosen_panel, NoteKind.Mine)])

    return mine_beat

def lastNoteMine():
    chosen_panel = random.choice(list(player.getLastFoot().getPanels()))
...
def weakUncrossPanel(step_foot):
    ...
    if step_foot == Foot.L:
        if player.footPanels(Foot.R) == [unturn_right]: uncross_panel = unturn_left
        else: failure = True
    else:
        if player.footPanels(Foot.L) == [unturn_left]: uncross_panel = unturn_right
        else: failure = True

    if failure: blowup("There's no weak crossover panel available.")
...
def strongUncrossPanel(step_foot):
    ...
    if unturn_up in current_panels: return unturn_down
    elif unturn_down in current_panels: return unturn_up
    else: blowup("Strong uncrossover is impossible here")

def randomJump(corner = False):
    # Output: A random jump, correcting for there being two up-downs. Can specify to use corners only
    return random.choice(ALL_CORNERS) if corner else random.choices(ALL_JUMPS, weights = [1, 1, 1, 0.5, 1, 0.5, 1])[0]

def jumpToBeat(jump):
    left_note = Note(jump[0], NoteKind.Tap)
    right_note = Note(jump[1], NoteKind.Tap)
...
def feetStuck(doublestep = False):
    ...
    if doublestep:
        ...
        def footStuck(foot):
            return player.getFoot(foot).anyHolds() or \
                player.last_foot == toLastFoot(foot) and not player.getFoot(not foot).anyHolds()

    return footStuck(Foot.L), footStuck(Foot.R)

def beforeLastFootBeat(): return player.getFoot(not toFoot(player.last_foot))

def ambiguousPanels():
    # TODO: Allow for jumps into up/down if a foot is on there
    return [unturnPanel(Panel.U, player.direction), unturnPanel(Panel.D, player.direction)]

def nextFoot(left_stuck, right_stuck):
    return twoBoolEvaluate(left_stuck, right_stuck, 
        [random.choice([Foot.L, Foot.R]), Foot.R, Foot.L, 'DIE'])
...
def stuckPanels(left_stuck, right_stuck):
    ...
    stuck_foot_beat = match([left_stuck, player.getFoot(Foot.L)], [right_stuck, player.getFoot(Foot.R)], default = Beat([]))
...
def setFeetBeat(*beats):
    for foot, beat in zip(Foot, beats): player.setFoot(foot, beat)
...
def crossDirection(step_foot):
    ...
    else: player.direction = turnRight(player.direction)
...
def switchDirection(step_foot):
    ...
    if step_foot == Foot.L: player.direction = turnRight(player.direction)
    else: player.direction = turnLeft(player.direction)

def jumpToNotes(jump, note_kind, length):
    return Note(unturnPanel(jump[0], player.direction), note_kind, length), Note(unturnPanel(jump[1], player.direction), note_kind, length)