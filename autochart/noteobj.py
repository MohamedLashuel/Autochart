...
class Note:
	@staticmethod
	def default():
		return Note(Panel.N, NoteType.NA, -1)

	def __init__(self, panel: Panel, kind: NoteType, length=-1):
		if length > -1:
			assert kind in [NoteType.Hold, NoteType.Roll]
		else:
			assert kind not in [NoteType.Hold, NoteType.Roll]
		assert length != 0
		self.panel = panel
		self.kind = kind
		self.length = length
...
class Beat:
	...
	def __add__(self, other):
		...
		for note in other.notes:
			new_beat.addNote(note)
		return new_beat

	def noteVars(self, var):
		return [getattr(note, var) for note in self.notes]
	...
	def anyHolds(self):
		return any(timer > 0 for timer in self.noteVars("length"))

	def getPanels(self):
		return set(self.noteVars("panel"))

	def advanceAndGetReleases(self):
		for note in self.notes:
			note.length = max(-1, note.length - 1)
		release_notes = [
			Note(note.panel, NoteType.Release)
			for note in self.notes
			if note.length == 0
		]
	...
	def Hflipped(self):
		return self.getSwappedIndicesBeat(0, 3)

	def Vflipped(self):
		return self.getSwappedIndicesBeat(1, 2)

	def toText(self):
		arrow_notes = [Note.default(), Note.default(), Note.default(), Note.default()]
		for note in self.notes:
			arrow_notes[note.panel - 1] = note

		arrow_strs = [note.kind.value for note in arrow_notes]

		return "".join(arrow_strs)

def changeNotePanel(beat: Beat, orig_pan: Panel, new_pan: Panel):
	panel_notes = [note for note in beat.notes if note.panel == orig_pan]
	if len(panel_notes):
		panel_notes[0].panel = new_pan

class Player:
	def __init__(self):
		self.feet = {Foot.L: Beat(), Foot.R: Beat()}
		self.last_foot = LastFoot.N
		self.direction = Facing.U

	def getPanels(self):
		return self.feet[Foot.L].getPanels() | self.feet[Foot.R].getPanels()

	def footPanels(self, foot):
		return self.feet[foot].getPanels()

	def getFoot(self, foot):
		return self.feet[foot]

	def setFoot(self, foot, beat):
		self.feet[foot] = beat

	def setLeft(self, beat):
		self.feet[Foot.L] = beat

	def setRight(self, beat):
		self.feet[Foot.R] = beat

	def getLastFoot(self):
		return self.feet[toFoot(self.last_foot)]
...
def flipLastFoot(last_foot):
	return matchVal(
		last_foot, [LastFoot.L, LastFoot.R], [LastFoot.R, LastFoot.L], default=last_foot
	)
